          ...
          for(p in t){
            // console.log(value);
             data[i].forEach((value,index)=>{
                if(t[p]==value||t[p]=='none'){
                    count++;
                }
            }); 
            const comp=(count/t.length)*100;
            console.log(comp);        
            if(comp>70){
                filter[i]=data[i];
                console.log((data[i] ,count/t.length)*100);
            }
        }
        }
        console.log(filter);
        if(t[t.length-1]=='none')
         return filter;
        if(Object.entries(filter).length === 1){
            return  Object.values(filter);
        }
        
        return (nextLayer(filter));
    }
    

    let nextfilter =new Set();
    function nextLayer(filter){
        for(i in filter){
            filter[i].forEach(k => {
                nextfilter.add(k);
            });
